<?
	...
	if($_POST && strlen(trim($_POST['location']))==0){
		include_once 'securimage/securimage.php';
		$securimage = new Securimage();
		if ($securimage->check($_POST['captchaTxt']) == false)
			print "<script>alert('Image code is incorrect. Please try again');</script>";
		else {
...
//			$to = $page->GetCampoValue(0, 'form_email');
			...
			$mm = new PHPMailer();
			$mm->IsSMTP();
			$mm->AddAddress('chad@lifestories.com');
			$mm->AddAddress('chadjharbour@yahoo.com');
			$mm->AddAddress('betabitsistemas@gmail.com');
...
<html>
    ...
    <body>
		...
		<div id="FirstSlide" class="carousel slide" data-ride="carousel" data-interval="false">
            <!-- Indicators -->
			...
			<div class="container">
				...
				<div id="SecondSlide" class="carousel slide" data-ride="carousel" data-interval="false">
					...
					<a class="right carousel-control" href="#SecondSlide" role="button" data-slide="next">
						<span class="glyphicon glyphicon-chevron-right" aria-hidden="true" style="color: #000000;"></span>
						<span class="sr-only">Next</span>
					</a>
			...
			<div class="container">
				...
				<div class="portfolio-memoirs">
					...
					if($detect->isMobile()){
								...
								<div class="modal-content">
								<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'.
									(strlen($portfolioMemoirs->GetCampoValue($key, 'content_image')) > 0?'<img src="updater/site_files/'.$portfolioMemoirs->GetCampoValue($key, 'content_image').'" style="max-width:100%">':'<h3>'.$portfolioMemoirs->GetCampoValue($key, 'title').'</h3><p>'.$portfolioMemoirs->GetCampoValue($key, 'description').'</p>').'
								</div>
								</div>
								</div>';
								...
								<div class="modal-content">
								<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'.
									(strlen($portfolioMemoirs->GetCampoValue($key, 'content_image')) > 0?'<img src="updater/site_files/'.$portfolioMemoirs->GetCampoValue($key, 'content_image').'" style="max-width:800px">':'<h3>'.$portfolioMemoirs->GetCampoValue($key, 'title').'</h3><p>'.$portfolioMemoirs->GetCampoValue($key, 'description').'</p>').'
								</div>
								</div>
								</div>';
					...
					<div class="portfolio-company">
						<h4>Company and Family History</h4><hr align="right">
						<?php
							$cont = 0;
							$cont1 = 1;
				...
				<div id="contact">
					...
					<div class="row">
						<div class="col-md-4"  style="padding-bottom: 10px;">
							...
							<input type="text" id="name" class="form-control" name="name" tabindex="1" required value="<?=$_POST['name']?>">
						</div>
						<div class="col-md-4" style="padding-bottom: 10px;">
							<label for="email">Email</label>
							<input type="email" id="email" class="form-control" name="email" tabindex="2" required value="<?=$_POST['email']?>">
						</div>
						<div class="col-md-4"  style="padding-bottom: 10px;">
							<label for="telephone">Telephone</label>
							<input type="text" id="telephone" class="form-control" name="telephone" tabindex="3" required value="<?=$_POST['telephone']?>">
						</div>
					</div>
					<div class="row">
						<div class="col-md-12" style="padding-bottom: 10px;">
							<label for="message">Message</label>
							<textarea name="message" class="form-control" rows="5" id="message" tabindex="4"><?=$_POST['message']?></textarea>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12" style="padding-bottom: 10px;">
							<div style="float:left">
								<img id="captcha" src="securimage/securimage_show.php" alt="Please enter this code" />
							</div>
							<div style="float:left;margin-left:10px">
								Please enter the code<br>
								<input type="text" name="captchaTxt" id="captchaTxt" size="7" maxlength="6" required class="form-control"  tabindex="5">
					...
					<div class="row">
						...
						<div class="col-md-4"><input type="text" id="location" class="form-control" name="location" tabindex="6" style="position:absolute;left:-10000px"></div>
					</div>
				</form>
			</div>
		</div>
