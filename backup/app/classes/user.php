...
class user
{
	 ...
	 * @return void
	 */
	function __construct($user_id='', $first_name, $last_name, $email, $password, $date='') {
	...
	public static function get_by_id($user_id) {
		$db = new db();
		$q = "SELECT user_id, first_name, last_name, email, password FROM users WHERE user_id = '$user_id'";
	...
	private static function get_by_email($email) {
		$db = new db();
		$q = "SELECT user_id, first_name, last_name, email, password FROM users WHERE email = '$email'";
		$u = $db->fetch( $db->query($q) );
		$user = new user(
					$user_id,
					$first_name,
	...
	private static function insert($user) {
		$db = new db();
		$q = "INSERT INTO users(first_name, last_name, email, password, date) 
		VALUES ('$user->first_name', '$user->last_name', '$user->email', '$user->password', '$user->date')";
		if( $db->query( $q ) ) {
			return true;
		} else {
			return false;
